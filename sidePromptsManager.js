...
function looksLikeV1(data) {
  ...
  return Object.values(data.prompts).some(
    (p) => p && typeof p === "object" && "type" in p && !("triggers" in p),
  );
}
...
function migrateV1toV2(v1) {
  ...
  for (const [key, p] of Object.entries(v1.prompts || {})) {
    const next = {
      ...
      enabled: !!p.enabled,
      prompt: String(
        p.prompt != null ? p.prompt : "this is a placeholder prompt",
      ),
    ...
    if (type === "tracker") {
      const intervalVisibleMessages = Math.max(
        1,
        Number(p.settings?.intervalVisibleMessages || 50),
      );
...
async function saveDoc(doc) {
  ...
  if (!res.ok) {
    throw new Error(
      __st_t_tag`Failed to save side prompts: ${res.status} ${res.statusText}`,
    );
  }

  cachedDoc = doc;
  console.log(
    `${MODULE_NAME}: ${translate("Side prompts saved successfully", "STMemoryBooks_SidePromptsSaved")}`,
  );
}
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (looksLikeV1(parsed)) {
        console.log(
          `${MODULE_NAME}: ${translate("Migrating side prompts file from V1(type) to V2(triggers)", "STMemoryBooks_MigratingSidePrompts")}`,
        );
        ...
        if (!validateSidePromptsFileV2(parsed)) {
          console.warn(
            `${MODULE_NAME}: ${translate("Invalid side prompts file structure; recreating with built-ins", "STMemoryBooks_InvalidSidePromptsFile")}`,
          );
          data = createBaseDoc();
          await saveDoc(data);
        } else {
    ...
    console.warn(
      `${MODULE_NAME}: ${translate("Error loading side prompts; creating base doc", "STMemoryBooks_ErrorLoadingSidePrompts")}`,
      e,
    );
    data = createBaseDoc();
    await saveDoc(data);
...
export async function findTemplateByName(name) {
  ...
  for (const p of templates) {
    ...
    if (
      ...
      nameSlug.startsWith(targetSlug) ||
...
export async function upsertTemplate(input) {
  ...
  const finalName =
    requestedName ||
    (isNew
      ? translate("Untitled Side Prompt", "STMemoryBooks_UntitledSidePrompt")
      : cur?.name ||
        translate("Untitled Side Prompt", "STMemoryBooks_UntitledSidePrompt"));
  ...
  const next = {
    ...
    prompt: String(
      input.prompt != null
        ? input.prompt
        : prev?.prompt || "this is a placeholder prompt",
    ),
    ...
    triggers: input.triggers
      ? input.triggers
      : prev?.triggers || { commands: ["sideprompt"] },
  ...
  if (next.triggers.onInterval) {
    const vis = Math.max(
      1,
      Number(next.triggers.onInterval.visibleMessages || 50),
    );
    next.triggers.onInterval = { visibleMessages: vis };
  }
  if (next.triggers.onAfterMemory) {
    next.triggers.onAfterMemory = {
      enabled: !!next.triggers.onAfterMemory.enabled,
    };
  }
  if ("commands" in next.triggers) {
    if (Array.isArray(next.triggers.commands)) {
      next.triggers.commands = next.triggers.commands.filter(
...
export async function removeTemplate(key) {
  const data = await loadSidePrompts();
  if (!data.prompts[key])
    throw new Error(__st_t_tag`Template "${key}" not found`);
...
export async function importFromJSON(jsonString) {
  ...
  if (validateSidePromptsFileV2(parsed)) {
    ...
    throw new Error(
      translate(
        "Invalid side prompts file structure",
        "STMemoryBooks_InvalidSidePromptsJSON",
      ),
    );
  }
  ...
  const merged = {
    version: Math.max(
      2,
      Number(existing.version || 2),
      Number(incoming.version || 2),
    ),
  ...
  const ensureUniqueKey = (desiredKey, name) => {
    ...
    let candidate =
      desiredKey && !merged.prompts[desiredKey] ? desiredKey : base;
  ...
  for (const [key, p] of Object.entries(incoming.prompts || {})) {
    ...
    const finalKey = merged.prompts[key]
      ? ensureUniqueKey(null, p?.name || key)
      : key;
    if (finalKey !== key) {
      renamed++;
    }
...
export async function recreateBuiltInSidePrompts(mode = "overwrite") {
  if (mode !== "overwrite") {
    console.warn(
      `${MODULE_NAME}: Unsupported mode for recreateBuiltInSidePrompts: ${mode}; defaulting to 'overwrite'`,
    );
  }
  ...
  cachedDoc = doc;
  console.log(
    `${MODULE_NAME}: Recreated built-in side prompts (overwrote ${replaced} entries)`,
  );
  return { replaced };
}
...
export async function listEnabledByType(type) {
  ...
  if (t === "tracker") {
    return all.filter(
      (p) =>
        p.enabled &&
        p.triggers?.onInterval &&
        Number(p.triggers.onInterval.visibleMessages) >= 1,
    );
  }
  if (t === "plotpoints") {
  ...
  if (t === "scoreboard") {
    ...
    return all.filter(
      (p) =>
        p.enabled &&
        (Array.isArray(p.triggers?.commands) ||
          p.triggers?.onAfterMemory?.enabled),
    );
  }
  return [];
}
...
export async function listByTrigger(kind) {
  ...
  if (kind && kind.startsWith("command:")) {
    ...
    return all.filter(
      (p) =>
        Array.isArray(p.triggers?.commands) &&
        p.triggers.commands.some((c) => c.toLowerCase() === cmd.toLowerCase()),
    );
  }
  return [];
}
