...
function getTriggersSummary(tpl) {
  ...
  if (trig.onInterval && Number(trig.onInterval.visibleMessages) >= 1) {
    badges.push(
      `${translate("Interval", "STMemoryBooks_Interval")}:${Number(trig.onInterval.visibleMessages)}`,
    );
  }
...
async function refreshList(popup, preserveKey = null) {
  ...
  const searchTerm = (
    popup?.dlg?.querySelector("#stmb-sp-search")?.value || ""
  ).toLowerCase();
  ...
  if (preserveKey) {
    const row = listContainer.querySelector(
      `tr[data-tpl-key="${CSS.escape(preserveKey)}"]`,
    );
    if (row) {
...
async function openEditTemplate(parentPopup, key) {
  try {
    ...
    if (!tpl) {
      toastr.error(
        tr("STMemoryBooks_TemplateNotFound", 'Template "{{key}}" not found', {
          key,
        }),
    ...
    const intervalEnabled = !!(
      trig.onInterval && Number(trig.onInterval.visibleMessages) >= 1
    );
    const intervalVal = intervalEnabled
      ? Math.max(1, Number(trig.onInterval.visibleMessages))
      : 50;
    ...
    let idx = Number.isFinite(s.overrideProfileIndex)
      ? Number(s.overrideProfileIndex)
      : (extension_settings?.STMemoryBooks?.defaultProfile ?? 0);
    ...
    const prevMemCount = Number.isFinite(s.previousMemoriesCount)
      ? Number(s.previousMemoriesCount)
      : 0;
    ...
    const lbOrderValue = Number.isFinite(lb.orderValue)
      ? Number(lb.orderValue)
      : 100;
...
<input type="number" id="stmb-sp-edit-prev-mem-count" class="text_pole" min="0" max="7" step="1" value="${prevMemCount}">
    ...
    const attachHandlers = () => {
      ...
      const intervalCont = dlg.querySelector(
        "#stmb-sp-edit-interval-container",
      );
      cbInterval?.addEventListener("change", () => {
        if (intervalCont)
          intervalCont.style.display = cbInterval.checked ? "block" : "none";
        if (cbInterval.checked)
          dlg.querySelector("#stmb-sp-edit-interval")?.focus();
      });

      const cbOverride = dlg.querySelector("#stmb-sp-edit-override-enabled");
      const overrideCont = dlg.querySelector(
        "#stmb-sp-edit-override-container",
      );
      cbOverride?.addEventListener("change", () => {
        if (overrideCont)
          overrideCont.style.display = cbOverride.checked ? "block" : "none";
      });
      ...
      const orderValCont = dlg.querySelector(
        "#stmb-sp-edit-lb-order-value-container",
      );
      const syncOrderVisibility = () => {
        if (orderValCont)
          orderValCont.style.display = orderManual?.checked ? "block" : "none";
      };
      ...
      const outletCont = dlg.querySelector(
        "#stmb-sp-edit-lb-outlet-name-container",
      );
      posSel?.addEventListener("change", () => {
        if (outletCont)
          outletCont.style.display = posSel.value === "7" ? "block" : "none";
      });
    };
    ...
    if (result === POPUP_RESULT.AFFIRMATIVE) {
      const dlg = editPopup.dlg;
      const newName =
        dlg.querySelector("#stmb-sp-edit-name")?.value.trim() || "";
      const newPrompt =
        dlg.querySelector("#stmb-sp-edit-prompt")?.value.trim() || "";
      const newResponseFormat =
        dlg.querySelector("#stmb-sp-edit-response-format")?.value.trim() || "";
      ...
      if (!newName) {
        toastr.info(
          translate(
            "Name was empty. Keeping previous name.",
            "STMemoryBooks_NameEmptyKeepPrevious",
          ),
          translate("STMemoryBooks", "index.toast.title"),
        );
      }
      ...
      // Triggers
      const triggers = {};
      const intervalOn = !!dlg.querySelector("#stmb-sp-edit-trg-interval")
        ?.checked;
      const afterOn = !!dlg.querySelector("#stmb-sp-edit-trg-aftermem")
        ?.checked;
      const manualOn = !!dlg.querySelector("#stmb-sp-edit-trg-manual")?.checked;

      if (intervalOn) {
        const intervalRaw = parseInt(
          dlg.querySelector("#stmb-sp-edit-interval")?.value ?? "50",
          10,
        );
      ...
      const overrideEnabled2 = !!dlg.querySelector(
        "#stmb-sp-edit-override-enabled",
      )?.checked;
      settings.overrideProfileEnabled = overrideEnabled2;
      if (overrideEnabled2) {
        const oidx = parseInt(
          dlg.querySelector("#stmb-sp-edit-override-index")?.value ?? "",
          10,
        );
      ...
      // Lorebook settings
      const lbModeSel =
        dlg.querySelector("#stmb-sp-edit-lb-mode")?.value || "link";
      const lbPosRaw = parseInt(
        dlg.querySelector("#stmb-sp-edit-lb-position")?.value ?? "0",
        10,
      );
      const lbOrderManual2 = !!dlg.querySelector(
        "#stmb-sp-edit-lb-order-manual",
      )?.checked;
      const lbOrderValRaw = parseInt(
        dlg.querySelector("#stmb-sp-edit-lb-order-value")?.value ?? "100",
        10,
      );
      const lbPrevent2 = !!dlg.querySelector("#stmb-sp-edit-lb-prevent")
        ?.checked;
      const lbDelay2 = !!dlg.querySelector("#stmb-sp-edit-lb-delay")?.checked;
      const outletNameVal =
        lbPosRaw === 7
          ? dlg.querySelector("#stmb-sp-edit-lb-outlet-name")?.value?.trim() ||
            ""
          : "";

      const prevCountRaw = parseInt(
        dlg.querySelector("#stmb-sp-edit-prev-mem-count")?.value ?? "0",
        10,
      );
      settings.previousMemoriesCount =
        Number.isFinite(prevCountRaw) && prevCountRaw > 0
          ? Math.min(prevCountRaw, 7)
          : 0;

      settings.lorebook = {
        constVectMode: ["link", "green", "blue"].includes(lbModeSel)
          ? lbModeSel
          : "link",
      ...
      toastr.success(
        tr(
          "STMemoryBooks_Toast_SidePromptUpdated",
          'SidePrompt "{{name}}" updated.',
    ...
    toastr.error(
      translate(
        "Failed to edit SidePrompt",
        "STMemoryBooks_FailedToEditSidePrompt",
      ),
      translate("STMemoryBooks", "index.toast.title"),
    );
  }
...
<input type="number" id="stmb-sp-new-prev-mem-count" class="text_pole" min="0" max="7" step="1" value="0">
  ...
  const attachHandlers = () => {
    ...
    cbInterval?.addEventListener("change", () => {
      if (intervalCont)
        intervalCont.style.display = cbInterval.checked ? "block" : "none";
      if (cbInterval.checked)
        dlg.querySelector("#stmb-sp-new-interval")?.focus();
    });
    ...
    const orderValCont = dlg.querySelector(
      "#stmb-sp-new-lb-order-value-container",
    );
    const syncOrderVisibility = () => {
      if (orderValCont)
        orderValCont.style.display = orderManual?.checked ? "block" : "none";
    ...
    const outletContNew = dlg.querySelector(
      "#stmb-sp-new-lb-outlet-name-container",
    );
    posSelNew?.addEventListener("change", () => {
      if (outletContNew)
        outletContNew.classList.toggle("displayNone", posSelNew.value !== "7");
    });
  };
  ...
  if (result === POPUP_RESULT.AFFIRMATIVE) {
    ...
    const responseFormat =
      dlg.querySelector("#stmb-sp-new-response-format")?.value.trim() || "";
    ...
    if (!name) {
      toastr.info(
        tr(
          ...
          'No name provided. Using "{{name}}".',
          {
            name: translate(
              "Untitled Side Prompt",
              "STMemoryBooks_UntitledSidePrompt",
            ),
          },
        ),
        translate("STMemoryBooks", "index.toast.title"),
    ...
    // Build triggers
    const triggers = {};
    const intervalOn = !!dlg.querySelector("#stmb-sp-new-trg-interval")
      ?.checked;
    ...
    if (intervalOn) {
      const intervalRaw = parseInt(
        dlg.querySelector("#stmb-sp-new-interval")?.value ?? "50",
        10,
      );
      const vis = Math.max(1, isNaN(intervalRaw) ? 50 : intervalRaw);
      triggers.onInterval = { visibleMessages: vis };
    ...
    // Settings - overrides
    const settings = {};
    const overrideEnabled = !!dlg.querySelector("#stmb-sp-new-override-enabled")
      ?.checked;
    settings.overrideProfileEnabled = overrideEnabled;
    if (overrideEnabled) {
      const oidx = parseInt(
        dlg.querySelector("#stmb-sp-new-override-index")?.value ?? "",
        10,
      );
      if (!isNaN(oidx)) settings.overrideProfileIndex = oidx;
    }
    ...
    // Settings - lorebook
    const lbModeSel =
      dlg.querySelector("#stmb-sp-new-lb-mode")?.value || "link";
    const lbPosRaw = parseInt(
      dlg.querySelector("#stmb-sp-new-lb-position")?.value ?? "0",
      10,
    );
    const lbOrderManual2 = !!dlg.querySelector("#stmb-sp-new-lb-order-manual")
      ?.checked;
    const lbOrderValRaw = parseInt(
      dlg.querySelector("#stmb-sp-new-lb-order-value")?.value ?? "100",
      10,
    );
    ...
    const outletNameVal =
      lbPosRaw === 7
        ? dlg.querySelector("#stmb-sp-new-lb-outlet-name")?.value?.trim() || ""
        : "";

    const prevCountRaw = parseInt(
      dlg.querySelector("#stmb-sp-new-prev-mem-count")?.value ?? "0",
      10,
    );
    settings.previousMemoriesCount =
      Number.isFinite(prevCountRaw) && prevCountRaw > 0
    ...
    try {
      ...
      toastr.success(
        translate("SidePrompt created", "STMemoryBooks_SidePromptCreated"),
        translate("STMemoryBooks", "index.toast.title"),
      );
      ...
      toastr.error(
        translate(
          "Failed to create SidePrompt",
          "STMemoryBooks_FailedToCreateSidePrompt",
        ),
        translate("STMemoryBooks", "index.toast.title"),
      );
    }
...
async function exportTemplates() {
  try {
    ...
    toastr.success(
      translate(
        "Side prompts exported successfully",
        "STMemoryBooks_SidePromptsExported",
      ),
      translate("STMemoryBooks", "index.toast.title"),
    );
  } catch (err) {
    ...
    toastr.error(
      translate(
        ...
        "STMemoryBooks_FailedToExportSidePrompts",
      ),
      translate("STMemoryBooks", "index.toast.title"),
    );
  }
...
async function importTemplates(event, parentPopup) {
  ...
  try {
    ...
    if (res && typeof res === "object") {
      ...
      toastr.success(
        translate("Imported side prompts", "STMemoryBooks_ImportedSidePrompts"),
        translate("STMemoryBooks", "index.toast.title"),
      );
    }
    await refreshList(parentPopup);
    ...
    toastr.error(
      tr(
        "STMemoryBooks_FailedToImportSidePrompts",
...
export async function showSidePromptsPopup() {
  try {
    let content =
      '<h3 data-i18n="STMemoryBooks_SidePrompts_Title">🧩 Side Prompts</h3>';
    content += '<div class="world_entry_form_control">';
    content +=
      '<p class="opacity70p" data-i18n="STMemoryBooks_SidePrompts_Desc">Create and manage side prompts for trackers and other behind-the-scenes functions.</p>';
    content += "</div>";

    // Search/filter box
    content += '<div class="world_entry_form_control">';
    content +=
      '<input type="text" id="stmb-sp-search" class="text_pole" data-i18n="[placeholder]STMemoryBooks_SearchSidePrompts;[aria-label]STMemoryBooks_SearchSidePrompts" placeholder="Search side prompts..." aria-label="Search side prompts" />';
    content += "</div>";
    ...
    content +=
      '<input type="number" id="stmb-sp-max-concurrent" class="text_pole" min="1" max="5" step="1" value="2">';
    ...
    const attachHandlers = () => {
      ...
      if (spMaxInput) {
        ...
        const persist = () => {
          ...
          if (!extension_settings.STMemoryBooks.moduleSettings)
            extension_settings.STMemoryBooks.moduleSettings = {};
          extension_settings.STMemoryBooks.moduleSettings.sidePromptsMaxConcurrent =
            val;
      ...
      // Search
      dlg
        .querySelector("#stmb-sp-search")
        ?.addEventListener("input", () => refreshList(popup));
      ...
      dlg
        .querySelector("#stmb-sp-import-file")
      ...
      dlg
        .querySelector("#stmb-sp-recreate-builtins")
        ?.addEventListener("click", async () => {
          ...
          const confirmPopup = new Popup(
            ...
            "",
            {
              okButton: translate(
                "Recreate",
                "STMemoryBooks_SidePrompts_RecreateOk",
              ),
              cancelButton: translate("Cancel", "STMemoryBooks_Cancel"),
            },
          );
          ...
          if (res === POPUP_RESULT.AFFIRMATIVE) {
            try {
              ...
              console.error(
                "STMemoryBooks: Error recreating built-in side prompts:",
                err,
              );
              toastr.error(
                translate(
        ...
        if (actionBtn) {
          ...
          if (actionBtn.classList.contains("stmb-sp-action-edit")) {
            ...
            try {
              ...
              toastr.success(
                translate(
                  "SidePrompt duplicated",
                  "STMemoryBooks_SidePromptDuplicated",
                ),
                translate("STMemoryBooks", "index.toast.title"),
              );
              ...
              console.error(
                "STMemoryBooks: Error duplicating side prompt:",
                err,
              );
              toastr.error(
                translate(
                  "Failed to duplicate SidePrompt",
                  "STMemoryBooks_FailedToDuplicateSidePrompt",
                ),
                translate("STMemoryBooks", "index.toast.title"),
              );
            }
            ...
            if (res === POPUP_RESULT.AFFIRMATIVE) {
              try {
                ...
                toastr.success(
                  translate(
                    "SidePrompt deleted",
                    "STMemoryBooks_SidePromptDeleted",
                  ),
                  translate("STMemoryBooks", "index.toast.title"),
                );
                await refreshList(popup);
                ...
                console.error(
                  "STMemoryBooks: Error deleting side prompt:",
                  err,
                );
                toastr.error(
                  translate(
                    "Failed to delete SidePrompt",
                    "STMemoryBooks_FailedToDeleteSidePrompt",
                  ),
                  translate("STMemoryBooks", "index.toast.title"),
                );
              }
