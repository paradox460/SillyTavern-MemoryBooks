...
export async function editProfile(settings, profileIndex, refreshCallback) {
  ...
  try {
    ...
    const templateData = {
      ...
      hasLegacyCustomPrompt:
        profile.prompt && profile.prompt.trim() ? true : false,
    };

    const content = DOMPurify.sanitize(profileEditTemplate(templateData));

    const popupInstance = new Popup(
      `<h3>${translate("Edit Profile", "STMemoryBooks_ProfileEditTitle")}</h3>${content}`,
      POPUP_TYPE.TEXT,
      "",
      {
    ...
    if (result === POPUP_RESULT.AFFIRMATIVE) {
      ...
      if (!validateProfile(updatedProfile)) {
        toastr.error(
          translate("Invalid profile data", "STMemoryBooks_InvalidProfileData"),
          "STMemoryBooks",
        );
        return;
      }
      ...
      toastr.success(
        translate(
          ...
          "STMemoryBooks_ProfileUpdatedSuccess",
        ),
        "STMemoryBooks",
      );
    }
    ...
    toastr.error(
      translate("Failed to edit profile", "STMemoryBooks_FailedToEditProfile"),
      "STMemoryBooks",
    );
  }
}
...
export async function newProfile(settings, refreshCallback) {
  try {
    ...
    const popupInstance = new Popup(
      `<h3>${translate("New Profile", "STMemoryBooks_NewProfileTitle")}</h3>${content}`,
      POPUP_TYPE.TEXT,
      "",
      {
    ...
    if (result === POPUP_RESULT.AFFIRMATIVE) {
      const newProfileData = buildProfileFromForm(
        popupInstance.dlg,
        defaultName,
      );

      const finalName = generateSafeProfileName(
        newProfileData.name,
        existingNames,
      );
      ...
      toastr.success(
        translate(
          ...
          "STMemoryBooks_ProfileCreatedSuccess",
        ),
        "STMemoryBooks",
      );
    }
...
export async function deleteProfile(settings, profileIndex, refreshCallback) {
  if (settings.profiles.length <= 1) {
    toastr.error(
      translate(
        ...
        "STMemoryBooks_CannotDeleteLastProfile",
      ),
      "STMemoryBooks",
    );
    return;
  ...
  try {
    const confirmText = translate(
      'Delete profile "{{name}}"?',
      "STMemoryBooks_DeleteProfileConfirm",
    ...
    if (result === POPUP_RESULT.AFFIRMATIVE) {
      ...
      toastr.success(
        translate(
          ...
          "STMemoryBooks_ProfileDeletedSuccess",
        ),
        "STMemoryBooks",
      );
    }
  ...
  try {
    ...
    toastr.success(
      translate(
        ...
        "STMemoryBooks_ProfilesExportedSuccess",
      ),
      "STMemoryBooks",
    );
  } catch (error) {
...
export function importProfiles(event, settings, refreshCallback) {
    ...
    try {
      ...
      if (importedCount > 0) {
        ...
        toastr.success(
          message,
          translate(
            "STMemoryBooks profile import completed",
      ...
      toastr.error(
        __st_t_tag`Failed to import profiles: ${error.message}`,
        "STMemoryBooks",
      );
    }
  };
  ...
  reader.onerror = function () {
    ...
    toastr.error(
      translate("Failed to read import file", "STMemoryBooks_ImportReadError"),
      "STMemoryBooks",
    );
  };
...
function setupProfileEditEventHandlers(popupInstance, settings) {
  ...
  popupElement
    .querySelector("#stmb-open-prompt-manager")
    ?.addEventListener("click", () => {
      try {
        ...
        console.error(
          `${MODULE_NAME}: Error opening prompt manager from profile editor:`,
          err,
        );
        toastr.error(
          translate(
  ...
  popupElement
    .querySelector("#stmb-refresh-presets")
    ?.addEventListener("click", async () => {
      try {
        ...
        if (
          ![...selectEl.options].some((o) => o.value === prev) &&
        ...
        toastr.error(
          translate(
            ...
            "STMemoryBooks_FailedToRefreshPresets",
          ),
          "STMemoryBooks",
        );
      }
  ...
  const stmbOnPresetsUpdated = async () => {
    try {
      ...
      if (
        ![...selectEl2.options].some((o) => o.value === prev2) &&
        selectEl2.options.length > 0
      ) {
        selectEl2.selectedIndex = 0;
      }
    } catch (e) {
      console.error(
        `${MODULE_NAME}: Error auto-refreshing presets on update:`,
        e,
      );
    }
  };
  ...
  popupElement
    .querySelector("#stmb-move-to-preset")
    ?.addEventListener("click", async () => {
      try {
        const legacyPromptEl = popupElement.querySelector(
          "#stmb-legacy-custom-prompt",
        );
        const legacyPrompt = legacyPromptEl ? legacyPromptEl.textContent : "";
        if (!legacyPrompt || !legacyPrompt.trim()) {
          toastr.error(
            t(
              "STMemoryBooks_NoCustomPromptToMigrate",
              "No custom prompt to migrate",
            ),
            "STMemoryBooks",
          );
          return;
        ...
        const profileNameInput =
          popupElement.querySelector("#stmb-profile-name");
        const profileName = profileNameInput?.value?.trim() || "Profile";
        const displayName = `Custom: ${profileName}`;

        const newKey = await SummaryPromptManager.upsertPreset(
          null,
          legacyPrompt,
          displayName,
        );
        ...
        console.error(
          `${MODULE_NAME}: Error moving custom prompt to preset:`,
          error,
        );
        toastr.error(
          translate(
  ...
  popupElement
    .querySelector("#stmb-profile-title-format-select")
    ?.addEventListener("change", (e) => {
      const customInput = popupElement.querySelector(
        "#stmb-profile-custom-title-format",
      );
  ...
  popupElement
    .querySelector("#stmb-profile-temperature")
  ...
  popupElement
    .querySelector("#stmb-profile-model")
    ?.addEventListener("input", (e) => {
      e.target.value = e.target.value.replace(/[<>]/g, "");
    });

  popupElement
    .querySelector("#stmb-profile-api")
    ?.addEventListener("change", (e) => {
      const fullManualSection = popupElement.querySelector(
        "#stmb-full-manual-section",
      );
  ...
  popupElement.querySelectorAll('input[name="order-mode"]').forEach((radio) => {
    radio.addEventListener("change", (e) => {
      const orderValueInput = popupElement.querySelector(
        "#stmb-profile-order-value",
      );
  ...
  positionSelect?.addEventListener("change", () => {
    const cont = popupElement.querySelector(
      "#stmb-profile-outlet-name-container",
    );
    if (cont)
  ...
  try {
    const recursionHeader = popupElement.querySelector(
      'h4[data-i18n="STMemoryBooks_RecursionSettings"]',
    );
    const recursionBlock = recursionHeader
      ? recursionHeader.parentElement?.querySelector(".buttons_block")
      : null;
    if (
      recursionBlock &&
      !popupElement.querySelector("#stmb-convert-existing-recursion")
    ) {
      ...
      input.addEventListener("change", () => {
        try {
          ...
          console.error(
            `${MODULE_NAME}: Failed to save convertExistingRecursion flag`,
            e,
          );
        }
      });
    ...
    console.warn(
      `${MODULE_NAME}: Failed to inject convertExistingRecursion checkbox`,
      e,
    );
  }
}
...
function buildProfileFromForm(popupElement, fallbackName) {
  ...
  const data = {
    name:
      popupElement.querySelector("#stmb-profile-name")?.value.trim() ||
      fallbackName,
    ...
    constVectMode: popupElement.querySelector("#stmb-profile-const-vect")
      ?.value,
    position: popupElement.querySelector("#stmb-profile-position")?.value,
    orderMode: popupElement.querySelector('input[name="order-mode"]:checked')
      ?.value,
    orderValue: popupElement.querySelector("#stmb-profile-order-value")?.value,
    preventRecursion: popupElement.querySelector(
      "#stmb-profile-prevent-recursion",
    )?.checked,
    delayUntilRecursion: popupElement.querySelector(
      "#stmb-profile-delay-recursion",
    )?.checked,
  };
  ...
  const titleFormatSelect = popupElement.querySelector(
    "#stmb-profile-title-format-select",
  );
  if (titleFormatSelect?.value === "custom") {
    data.titleFormat = popupElement.querySelector(
      "#stmb-profile-custom-title-format",
    )?.value;
  ...
  if (data.position === "7" || data.position === 7) {
    data.outletName =
      popupElement.querySelector("#stmb-profile-outlet-name")?.value?.trim() ||
      "";
  }
...
export function validateAndFixProfiles(settings) {
  ...
  if (settings.profiles.length === 0) {
    ...
    fixes.push(
      'Added default profile using provider "Current SillyTavern Settings".',
    );
  }
  ...
  settings.profiles = settings.profiles.map((p) => {
    if (p && p.useDynamicSTSettings) {
      ...
      fixes.push(
        `Migrated legacy dynamic profile "${p.name}" to provider-based current_st`,
      );
    }
    return p;
  });
  ...
  settings.profiles.forEach((profile, index) => {
    ...
    if (profile.preventRecursion === undefined) {
      ...
      fixes.push(
        `Added default 'preventRecursion' to profile "${profile.name}"`,
      );
    }
    if (profile.delayUntilRecursion === undefined) {
      profile.delayUntilRecursion = true;
      fixes.push(
        `Added default 'delayUntilRecursion' to profile "${profile.name}"`,
      );
    }
