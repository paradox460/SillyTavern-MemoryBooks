...
async function executeHideCommand(hideCommand, context = "") {
  ...
  console.log(
    i18n(
      ...
      `${MODULE_NAME}: Executing hide command${contextStr}: {{hideCommand}}`,
      { hideCommand },
    ),
  );
...
export async function addMemoryToLorebook(memoryResult, lorebookValidation) {
  try {
    if (!memoryResult?.content) {
      throw new Error(
        i18n(
          "addlore.errors.invalidContent",
          "Invalid memory result: missing content",
        ),
      );
    }
    ...
    if (!newEntry) {
      throw new Error(
        i18n(
          "addlore.errors.createEntryFailed",
          "Failed to create new lorebook entry",
        ),
      );
    }

    const entryTitle = generateEntryTitle(
      titleFormat,
      memoryResult,
      lorebookValidation.data,
    );
    ...
    if (settings.moduleSettings?.showNotifications !== false) {
      toastr.success(
        i18n(
          "addlore.toast.added",
          'Memory "{{entryTitle}}" added to "{{lorebookName}}"',
          { entryTitle: entryTitle, lorebookName: lorebookValidation.name },
        ),
        ...
        if (!sceneData) {
          console.warn(
            i18n(
              ...
              `${MODULE_NAME}: Auto-hide skipped - invalid scene range: "{{range}}"`,
              { range: memoryResult.metadata?.sceneRange },
            ),
          );
          toastr.warning(
            i18n(
          ...
          if (unhiddenCount === 0) {
            await executeHideCommand(
              `/hide 0-${sceneEnd}`,
              i18n("addlore.hideCommand.allComplete", "all mode - complete"),
            );
          } else {
            ...
            if (hideEndIndex >= 0) {
              await executeHideCommand(
                `/hide 0-${hideEndIndex}`,
                i18n("addlore.hideCommand.allPartial", "all mode - partial"),
              );
            }
        ...
        if (!sceneData) {
          ...
          if (unhiddenCount >= sceneSize) {
            ...
            await executeHideCommand(
              ...
              i18n("addlore.hideCommand.lastHideAll", "last mode - hide all"),
            );
          } else {
            const hideEnd = sceneEnd - unhiddenCount;
            if (hideEnd >= sceneStart) {
              await executeHideCommand(
                `/hide ${sceneStart}-${hideEnd}`,
                i18n("addlore.hideCommand.lastPartial", "last mode - partial"),
              );
            }
    ...
    return {
      ...
      message: i18n(
        "addlore.result.added",
        'Memory successfully added to "{{lorebookName}}"',
        { lorebookName: lorebookValidation.name },
      ),
    };
  } catch (error) {
    console.error(
      i18n(
        ...
        `${MODULE_NAME}: Failed to add memory to lorebook:`,
      ),
      error,
    );

    if (
      extension_settings.STMemoryBooks?.moduleSettings?.showNotifications !==
      false
    ) {
      toastr.error(
        i18n("addlore.toast.addFailed", "Failed to add memory: {{message}}", {
    ...
    return {
      ...
      error: error.message,
      message: i18n(
        "addlore.result.addFailed",
        "Failed to add memory to lorebook: {{message}}",
        { message: error.message },
      ),
    };
  }
 ...
 */
function populateLorebookEntry(
  entry,
  memoryResult,
  entryTitle,
  lorebookSettings,
  ...
  if (memoryResult.metadata?.sceneRange) {
    // Set metadata for scene range if available
...
function generateEntryTitle(titleFormat, memoryResult, lorebookData) {
  ...
  const substitutions = {
    "{{title}}":
      memoryResult.extractedTitle || i18n("addlore.defaults.title", "Memory"),
    "{{scene}}": i18n("addlore.defaults.scene", "Scene {{range}}", {
      range: metadata.sceneRange || i18n("common.unknown", "Unknown"),
    }),
...
function extractNumberFromTitle(title) {
  ...
  for (const match of allNumbers) {
    ...
    const isDateComponent =
      /\d{4}-\d{2}-\d{2}/.test(before + fullMatch + after) ||
...
export function getDefaultTitleFormats() {
  return DEFAULT_TITLE_FORMATS.map((fmt, idx) =>
    i18n(`addlore.titleFormats.${idx}`, fmt),
  );
}
...
export function validateTitleFormat(format) {
  ...
  const invalidPlaceholders = format
    .match(/\{\{[^}]*\}\}/g)
    ?.filter((placeholder) => {
  ...
  if (invalidPlaceholders && invalidPlaceholders.length > 0) {
    warnings.push(
      i18n(
        ...
        { placeholders: invalidPlaceholders.join(", ") },
      ),
    );
  }
  ...
  if (numberingPatterns) {
    ...
    const invalidNumbering = numberingPatterns.filter(
      (pat) => !allowed.some((rx) => rx.test(pat)),
    );
    ...
    if (invalidNumbering.length > 0) {
      warnings.push(
        i18n(
          ...
          "Invalid numbering patterns: {{patterns}}. Use [0], [00], [000], (0), {0}, #0, #[0], ([0]), {[0]} etc.",
          { patterns: invalidNumbering.join(", ") },
        ),
      );
    }
  }
...
export function previewTitle(titleFormat, sampleData = {}) {
  ...
  const mockLorebookData = {
    entries: {
      existing1: {
        uid: 5,
        comment: "[001] - Previous Memory",
        stmemorybooks: true,
      },
      existing2: {
        uid: 7,
        comment: "[002] - Another Memory",
        stmemorybooks: true,
      },
    },
  };
  ...
  try {
    ...
    return i18n("addlore.preview.error", "Error: {{message}}", {
      message: error.message,
    });
  }
}
...
export function getRangeFromMemoryEntry(entry) {
  if (
    typeof entry.STMB_start === "number" &&
    typeof entry.STMB_end === "number"
  ) {
  ...
  try {
    ...
    if (!lorebookName) {
      return {
        valid: false,
        error: i18n(
          "addlore.stats.errors.noBinding",
          "No lorebook bound to chat",
        ),
      };
    }
    ...
    if (!lorebookData) {
      return {
        valid: false,
        error: i18n(
          "addlore.stats.errors.loadFailed",
    ...
    const otherEntries = entries.filter(
      (entry) => !memoryEntries.some((memEntry) => memEntry.entry === entry),
    );
    ...
    return {
      ...
      averageContentLength:
        entries.length > 0
          ...
          : 0,
      totalKeywords: entries.reduce(
        (sum, entry) => sum + (entry.key?.length || 0),
        0,
      ),
      memoryKeywords: memoryEntries.reduce(
    ...
    console.error(
      i18n(
        ...
        `${MODULE_NAME}: Error getting lorebook stats:`,
      ),
      error,
    );
  ...
  try {
    console.log(
      i18n(
        ...
        `${MODULE_NAME}: updateHighestMemoryProcessed called with:`,
      ),
      memoryResult,
    );
    ...
    console.log(
      i18n(
        ...
        `${MODULE_NAME}: sceneRange extracted:`,
      ),
      sceneRange,
    );
    ...
    if (!sceneRange) {
      console.warn(
        i18n(
          ...
          `${MODULE_NAME}: No scene range found in memory result, cannot update highest processed`,
        ),
      );
      return;
    }
    ...
    if (rangeParts.length !== 2) {
      console.warn(
        i18n(
          ...
          `${MODULE_NAME}: Invalid scene range format: {{range}}`,
          { range: sceneRange },
        ),
      );
      return;
    ...
    if (isNaN(endMessage)) {
      console.warn(
        i18n(
          ...
          `${MODULE_NAME}: Invalid end message number: {{end}}`,
          { end: rangeParts[1] },
        ),
      );
      return;
    ...
    if (!sceneMarkers) {
      console.warn(
        i18n(
          ...
          `${MODULE_NAME}: Could not get scene markers to update highest processed`,
        ),
      );
      return;
    }
    ...
    console.log(
      i18n(
        ...
        `${MODULE_NAME}: Set highest memory processed to message {{endMessage}}`,
        { endMessage },
      ),
    );
  } catch (error) {
    console.error(
      i18n(
        ...
        `${MODULE_NAME}: Error updating highest memory processed:`,
      ),
      error,
    );
  }
 ...
 */
export async function upsertLorebookEntriesBatch(
  lorebookName,
  lorebookData,
  items,
  options = {},
) {
  const { refreshEditor = true } = options;

  if (!lorebookName || !lorebookData || !Array.isArray(items)) {
    throw new Error(
      i18n(
        "addlore.upsert.errors.invalidArgs",
        "Invalid arguments to upsertLorebookEntriesBatch",
      ),
    );
  }
  ...
  for (const it of items) {
    ...
    if (!entry) {
      ...
      if (!entry) {
        throw new Error(
          i18n(
            "addlore.upsert.errors.createFailed",
            "Failed to create lorebook entry",
          ),
        );
      }
      ...
      if (typeof defaults.position === "number")
        entry.position = defaults.position;
    ...
    entry.keysecondary = Array.isArray(entry.keysecondary)
      ? entry.keysecondary
      : [];
 ...
 */
export async function upsertLorebookEntryByTitle(
  lorebookName,
  lorebookData,
  title,
  content,
